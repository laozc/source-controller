...
func TestHelmRepository_reconcileSource(t *testing.T) {
	...
	tests := []struct {
		...
		beforeFunc       func(t *WithT, obj *sourcev1.HelmRepository, chartRepo *repository.ChartRepository)
		...
		{
			name:     "HTTP without secretRef makes Ready=True",
			protocol: "http",
		...
		},
		{
			...
			beforeFunc: func(t *WithT, obj *sourcev1.HelmRepository, chartRepo *repository.ChartRepository) {
				...
				obj.Status.Artifact = &sourcev1.Artifact{Checksum: checksum}
				conditions.MarkTrue(obj, meta.ReadyCondition, meta.SucceededReason, "Test artifact ready")
			},
			afterFunc: func(t *WithT, obj *sourcev1.HelmRepository) {
				t.Expect(obj.Status.Artifact.URL).To(BeEmpty())
				t.Expect(obj.Status.Artifact.Path).To(BeEmpty())
			},
			assertConditions: []metav1.Condition{
				*conditions.TrueCondition(meta.ReadyCondition, meta.SucceededReason, "Stored artifact for revision"),
			},
		...
		{
			name:     "HTTP with Basic Auth secret makes Ready=True",
			protocol: "http",
			server: options{
				username: "git",
				password: "1234",
		...
		{
			name:     "HTTPS with CAFile secret makes Ready=True",
			protocol: "https",
			server: options{
		...
		{
			name:     "Removes ArtifactOutdatedCondition after creating artifact",
			protocol: "http",
			beforeFunc: func(t *WithT, obj *sourcev1.HelmRepository, chartRepo *repository.ChartRepository) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Cleanup any chart index created for this chart repo.
			defer func() {
				os.RemoveAll(chartRepo.CachePath)
			}()

			if tt.beforeFunc != nil {
				tt.beforeFunc(g, obj, chartRepo)
			}
			...
			got, err := r.reconcileSource(logr.NewContext(ctx, log.NullLogger{}), obj)
			g.Expect(obj.Status.Conditions).To(conditions.MatchConditions(tt.assertConditions))
			g.Expect(err != nil).To(Equal(tt.wantErr))
			g.Expect(got).To(Equal(tt.want))
