...
func (r *HelmRepositoryReconciler) reconcile(ctx context.Context, obj *sourcev1.HelmRepository) (ctrl.Result, error) {
	...
	if result, err := r.reconcileSource(ctx, obj); err != nil || result.IsZero() {
		return result, err
	}
...
//
// The caller should assume a failure if an error is returned, or the Result is zero.
func (r *HelmRepositoryReconciler) reconcileSource(ctx context.Context, obj *sourcev1.HelmRepository) (ctrl.Result, error) {
	...
	chartRepo, err := repository.NewChartRepository(obj.Spec.URL, "", r.Getters, clientOpts)
	if err != nil {
		switch err.(type) {
		case *url.Error:
	...
	if obj.GetArtifact() != nil && obj.GetArtifact().Checksum == checksum {
		if artifact.URL != obj.GetArtifact().URL {
			r.Storage.SetArtifactURL(obj.GetArtifact())
			obj.Status.URL = r.Storage.SetHostname(obj.Status.URL)
		}
		return ctrl.Result{RequeueAfter: obj.GetRequeueAfter()}, nil
	...
	if err := r.Storage.MkdirAll(artifact); err != nil {
		conditions.MarkTrue(obj, sourcev1.FetchFailedCondition, sourcev1.StorageOperationFailedReason,
			"Failed to create repository index directory: %s", err.Error())
		r.Eventf(ctx, obj, events.EventSeverityError, sourcev1.StorageOperationFailedReason,
			"Failed to create repository index directory: %s", err.Error())
		return ctrl.Result{}, err
	}
	...
	if err != nil {
		err = fmt.Errorf("unable to acquire lock for artifact: %w", err)
		return ctrl.Result{}, err
	}
	defer unlock()
	...
	if err != nil {
		...
		r.Eventf(ctx, obj, events.EventSeverityError, sourcev1.StorageOperationFailedReason,
			"Failed to update status URL symlink: %s", err.Error())
		return ctrl.Result{}, err
	}
